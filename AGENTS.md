...
- 普通版以 `main` 为唯一基线，实验源码在隔离目录中由 `SLEEPDOWN_EXP_BUILD` 关闭普通构建入口；历史 `exp` 分支继续从 `main` 同步，不维护第二套普通业务。普通功能先进入 `main`；版本、更新通道、组件和发布规则见 [普通版与实验版规范](docs/EXP_BRANCH_AND_RELEASES.md)。
