...
- 用户确认 `EA6C78A5...` 包横向 Pager 仍卡顿，Closing 主要是模糊卸载点跳变。最新源码将相邻周恢复为常驻材质、只保留纵向视口卸载；背景预建 32 档，Opening 保持原 12 档节奏，Closing 使用全部 32 档并在仍有 42% 模糊强度时从 0.5× 冻结纹理提前切回全分辨率单层，不做双层叠画。只构建启用大玻璃性能、关闭液态动效的 GitHub Release；Kotlin、R8、lintVital、打包和签名通过，大小 `6,446,118` bytes，SHA-256 `38F38439D90F5744DE52129450517115BE50461EE21703A6C69D853DD1D7483C`。构建后设备仍从 ADB 离线，安装返回 `no devices/emulators found`，因此该包尚未安装或启动。
...
5. 液态玻璃下一步只推进性能：冻结阶段三动效；设备恢复连接后先覆盖安装 `38F38439...` 包，由用户重点检查各实质遮挡路线的 Closing 末段是否还跳变，并观察相邻周常驻后 Pager 首拖是否减少分配型卡顿。若横向仍明显掉帧，下一步不再做横向卸载/恢复，而是把按星期列的局部合批提升为带面积效率约束的整页空间分块合批，避免双页同时可见时出现 14 条列级效果链；之后用诊断核对消费者层数、Offscreen 像素与 RenderTarget 尺寸。每批实际修改都必须开启对应开关构建并覆盖安装；设备离线时明确记录“构建完成、安装待办”。
