...
5. 液态玻璃下一步只推进性能：冻结阶段三动效；当前 `EA6C78A5...` 包已安装，由用户检查 12 卡左右 Pager/上下滚动、离屏返回首帧、Closing 尾段和个性化滑块实时预览。若仍明显掉帧，再把按星期列的局部合批提升为带面积效率约束的整页空间分块合批，避免一个跨满屏空隙的巨大 RenderTarget；之后用诊断核对消费者层数、Offscreen 像素与 RenderTarget 尺寸。每批实际修改都必须开启对应开关构建并覆盖安装；设备离线时必须明确记录“构建完成、安装待办”，不得谎记为已安装。
