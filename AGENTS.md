...
- 阶段三第一版的轻微速度/加速度 outline 形变已完整封存在本地分支 `codex/archive/three-dot-outline-motion-wip-20260823`（`1e86605`），源码中的原型函数也继续保留。当前第二版只实验首页三点菜单：菜单中心仍完全沿用已验收的 Legacy Bezier 轨迹、440/285ms 路径时间线、移动方向、真实三点按钮返回锚点和最终 194×317dp 位置；玻璃壳宽高/圆角改由 Issue #70 的准确 Compose `spring(dampingRatio=0.75, stiffness=200)` 驱动，内容 alpha 使用其 `progress 0.2→1.0` 交接。实现不逐帧改变 `.size()`，而是在覆盖整条路径和安全越界范围的固定 envelope 内画动态 rect/radius；内容始终按真实终态尺寸测量并居中，不非等比缩放文字或按钮，Open 继续由原 Kyant 30dp surface 绘制并释放转场 envelope。路线仍由默认 `false` 的 `sleepdown.enableLiquidMotionExperiment` 和 `home-three-dot-menu` allowlist 双重控制，与阶段二开关相互独立。
- 诊断 counter、实验边界、测试与后续启用门槛见 `docs/performance/LIQUID_GLASS_FRAMEWORK.md`。当前设备已覆盖安装只开启阶段三开关的精确 Issue #70 受控 Release；未由 Codex 启动或操作，后续真机性能/视觉测试仍需用户明确要求。
...
- 阶段二累计新增 4 项稳定 envelope 像素定位、路线门控、面积上限与 aura 几何测试；阶段三累计新增 3 项独立门控、旧轮廓端点及 Issue #70 精确弹簧/Legacy 路径中心不变测试。项目没有 Release unit-test task，且按“只构建 Release”约束未改跑 Debug，故这 7 项尚未计入已通过总数。阶段二实验 Release（SHA-256 `D47506F3E42A2177EC0482D6D14CCEA0AFC96D829623670186E9634BE0C12B87`）已覆盖安装，用户肉眼观察未发现明显帧率变化但未抓量化 trace。阶段三单开关 Release（SHA-256 `880BD142F469DEB46F2CDD0887FB3BD2350263E9D0821F5AA3F87E00D235070A`）仅构建未安装；随后同时开启 `sleepdown.enableLargeGlassExperiment=true` 与 `sleepdown.enableLiquidMotionExperiment=true` 的签名 GitHub Release 使用相同低并发、关闭资源压缩并保留 R8/lintVital 的配置构建通过，SHA-256 为 `CB0B395697DE0D714BBCD4A8BF9ED6B5BD53AEEEBE2B31A4F5A1660E099F81ED`，已覆盖安装到 PLJ110 `3B15AE023YL00000`，未启动或操作应用。本次精确 Issue #70 三点菜单版本继续只打开阶段三开关，Release/R8/lintVital 构建通过，SHA-256 为 `5995D781E910B007BF1E2AF821C9EBAA289BAA3EFBCD818EDE717AE01ABA68FA`，已覆盖安装到同一 PLJ110，未启动或操作应用。
...
5. 液态玻璃下一步先对阶段三第二版三点菜单做单路线主观录像验收：观察 Issue #70 式尺寸/圆角弹簧与内容渐入是否自然，同时确认移动方向、Legacy 路径中心、最终菜单位置和返回锚点不跳，文字/按钮不发生非等比形变。未通过可关闭实验开关回到 Legacy，或从 `codex/archive/three-dot-outline-motion-wip-20260823` 对照第一版轻微 outline。阶段二后续量化仍需验证首页三个菜单目的页及大屏个性化两个固定效果通道的像素一致性和 RenderTarget 尺寸是否稳定；课程编辑器仍须先实现并审计其专用动态 rect lens/SDF 与内容层生命周期。
