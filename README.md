...
SleepDown 围绕课表的导入、维护、提醒与日常查看进行设计，并提供桌面组件、今日助手和较完整的个性化能力。基础功能无需注册账号，课表、设置、壁纸与助手数据默认存储在设备本地。
...
支持手动录入，以及从教务系统、ICS 文件、文本、表格、图片和 PDF 导入课表。教务导入已适配武汉科技大学，同时保留通用教务页面作为补充；AI 导入可使用 OpenAI、DeepSeek、小米 MiMo 或自定义兼容接口。所有导入结果都会先经过预览确认，再写入本地课表。
